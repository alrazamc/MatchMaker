...
import { profileDescriptionoUpdated } from '../../../store/actions/ProfileActions';
...
const onSubmit = (values, dispatch, props) => {
  ...
  return getFirestore().collection('users').doc(props.auth.uid).set(update, {merge: true}).then(response => {
    props.formSubmitted();
    dispatch(profileDescriptionoUpdated());
  }).catch(err => {
...
const mapStateToProps = state => {
  return {
    ...
    initialValues: state.firebase.profile && state.firebase.profile.profileDescription ? {profileDescription: state.firebase.profile.profileDescription}  : undefined,
