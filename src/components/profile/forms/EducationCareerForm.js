...
import { profileEducationCareerUpdated } from '../../../store/actions/ProfileActions';
...
const onSubmit = (values, dispatch, props) => {
  ...
  return getFirestore().collection('users').doc(props.auth.uid).set(update, {merge: true}).then(response => {
    props.formSubmitted();
    dispatch(profileEducationCareerUpdated());
  }).catch(err => {
...
const mapStateToProps = state => {
  return {
    ...
    initialValues: state.firebase.profile ? state.firebase.profile.educationCareer : {},
