...
import { profileFamilyInfoUpdated } from '../../../store/actions/ProfileActions';
  ...
  return (
      <Box minWidth={280} maxWidth={400} my={2}>
        <form onSubmit={handleSubmit}>
          ...
          {
            formValues.motherStatus === '3'  ? // 2=Business
              <Box mb={2}>
                <Field 
                  component={TextInput}
...
const onSubmit = (values, dispatch, props) => {
  ...
  return getFirestore().collection('users').doc(props.auth.uid).set(update, {merge: true}).then(response => {
    props.formSubmitted();
    dispatch(profileFamilyInfoUpdated());
  }).catch(err => {
...
const mapStateToProps = state => {
  return {
    ...
    initialValues: state.firebase.profile ? state.firebase.profile.family : {},
