...
const PhotoGrid = ({ photos, uid, width, removePhotoFromProfile, changeProfilePicture }) => {
  ...
  const bucketRef = useMemo(() => {
    ...
    return storageRef.child(`images/${uid}/`);
  }, [firebase, uid]);
  ...
  const deleteImage = (index) => {
    ...
    bucketRef.child(images[index].file_name).delete().then(() => {
      removePhotoFromProfile(index);
      if(index === photos.profilePictureIndex)
        setProfilePicture(0);
      else if(index < photos.profilePictureIndex)
        setProfilePicture( photos.profilePictureIndex - 1 );
    }).catch(err => changeDeletePreloader(index, false))
  }
      ...
      <GridList cellHeight={160} cols={getGridListCols()}>
        {
          images.map((img, index) => (
            <GridListTile key={img.imgUrl} cols={1} className={classes.tile}>
              <img src={img.imgUrl} alt={uid} />
              <GridListTileBar
                title=""
                actionIcon={
                    img.deletePreloader ? 
...
const mapStateToProps = (state) => {
  return {
    uid: state.firebase.auth ? state.firebase.auth.uid : null,
    photos: state.firebase.profile && state.firebase.profile.photos ? state.firebase.profile.photos : {} 
  }
}
