...
export const profileBasicInfoSelector = (state) => {
  const basicInfo = {};
  if(!state.firebase.profile || !state.firebase.profile.basicInfo) return basicInfo;
  ...
  const info = state.firebase.profile.basicInfo;
  keys.forEach(key => {
...
export const profileLifestyleSelector = (state) => {
  const lifestyle = {};
  if(!state.firebase.profile || !state.firebase.profile.lifestyle) return lifestyle;
  const keys = ['diet', 'drink', 'smoke'];
  const { data } = systemSelector(state, [], keys);
  const info = state.firebase.profile.lifestyle;
  keys.forEach(key => {
...
export const profileReligionCasteSelector = (state) => {
  const religionCaste = {};
  if(!state.firebase.profile || !state.firebase.profile.religionCaste) return religionCaste;
  const keys = ['religions', 'communities', 'languages', 'namaaz', 'yesNo'];
  const { data } = systemSelector(state, [], keys);
  const info = state.firebase.profile.religionCaste;
...
export const profileFamilyInfoSelector = (state) => {
  let family = {};
  if(!state.firebase.profile || !state.firebase.profile.family) return family;
  const info = state.firebase.profile.family;
    family = {...info};
...
export const profileEducationCareerSelector = (state) => {
  let educationCareer = {};
  if(!state.firebase.profile || !state.firebase.profile.educationCareer) return educationCareer;
  const info = state.firebase.profile.educationCareer;
...
export const profileLocationSelector = (state) => {
  let location = {};
  if(!state.firebase.profile || !state.firebase.profile.location) return location;
  const info = state.firebase.profile.location;
    location = {...info};
